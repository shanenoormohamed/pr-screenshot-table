...
export function defaultRowTitle(index: number): string {
  return `Screen ${index + 1}`;
}

export function normalizeRowTitle(title: string | undefined, index: number): string {
  const screen = defaultRowTitle(index);
  if (!title || title === `Row ${index + 1}`) return screen;
  return title;
}

export function normalizeRowTitles(table: TableState): TableState {
  const rowTitles = table.rowTitles.map((title, index) =>
    normalizeRowTitle(title, index),
  );
...
export function createEmptyTable(rows: number, cols: number): TableState {
  return {
    ...
    rowTitles: Array.from({ length: rows }, (_, i) => defaultRowTitle(i)),
...
export function resizeTable(
  ...
  for (let r = 0; r < Math.min(rows, prev.rows); r += 1) {
    next.rowTitles[r] = normalizeRowTitle(prev.rowTitles[r], r);
